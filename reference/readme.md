...
3. **补充内容**：是否有官方网址、文档站、演示地址、参考项目/致谢对象、必须保留的文件索引，或其他一定要写进 README 的内容？

默认值：本地或对话草稿、不直接 push；文档型克制；不主动加 emoji、截图、Star History 或贡献图；只在仓库文件、用户输入或项目实际依赖/引用支持时加入官网、参考项目、致谢和文件索引。
