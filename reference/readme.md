...
6. 在开始写正文前，先用同一条消息问默认三问。没有明确授权时，不要直接上传或 push。
7. 估计输出规模。预计超过 120 行、4000 个中文字符或 2500 个英文词时，仍只问默认三问，并把长文取舍放进第 2、3 个问题中。用户明确要求“一次性直接写”或“别问”时可以继续，但要说明默认取舍。

除非用户明确说“直接写”“别问”“按你判断”，README 默认先问三问。问题必须放在同一条消息里，不要变成来回问卷。用户跳过提问时，先给可用草稿，并把不确定项标为 `TODO` 或“替换为你的仓库 URL”。

## 默认三问

README 有些选择会显著改变最终效果，默认先问 3 个问题；每个问题都应给出清晰选项，并允许用户补充。

1. **交付方式**：写成本地 markdown 文件、只给对话草稿，还是看过后再更新远端？
2. **风格和视觉**：偏文档型克制、社区/产品型活泼，还是沿用现有风格？标题或小节是否要 emoji，是否需要徽章、截图、Star History？
3. **补充内容**：是否有目标语言、官方网址、文档站、Demo、截图素材、参考项目/致谢对象、必须保留的文件索引，或任何一定要写进 README 的内容？

默认值：本地或对话草稿、不直接 push；文档型克制；不主动加 emoji、截图、Star History 或贡献图；README 语言默认使用英语和用户交流时使用的语言（如果二者相同，只输出英语）；只在仓库文件、用户输入或项目实际依赖/引用支持时加入官网、参考项目、致谢和文件索引。
...
- **多语言同步**：要求多语言时，各语言的结构、链接和命令保持一致；长 README 默认拆成多个文件。
...
- **引用有关系**：README 的参考项目、致谢和官网链接必须和目标项目真实相关。项目中提到、依赖、复用或借鉴的项目可以列；纯粹作为写作参考的仓库不要列进目标 README。
...
11. 参考项目 / 致谢：仅当仓库文件、依赖、用户输入或现有 README 明确提到外部项目、灵感来源、赞助、素材来源或重要上游时添加。
12. License：链接到许可证文件。
13. 交付方式：README 草稿阶段默认不直接执行远端发布；先把内容写出来让用户确认，再决定是否上传。
...
- 如果项目本身使用、改造、引用或明显受益于外部项目，可写 `Acknowledgements` / `References` / `Credits`；只列与目标项目直接相关的来源，并说明关系，例如 inspired by、built on、badge/source reference、asset source。
- 不把本 skill 的 `reference/source-catalog.md`、内部研究材料或无关示例仓库当成目标项目 README 的参考项目。
...
README 默认使用英语和用户交流时使用的语言。如果用户交流语言就是英语，只输出英语。询问语言时标注默认值，例如中文对话写“目标语言（默认：English + 简体中文）”；英语对话只写“目标语言（默认：English）”。

如果用户提到多语言版本但未指定语言，把“需要哪些语言”合并进 README 三问里的补充内容问题，不要自动生成大型语言集合。可把英语、简体中文、西班牙语、印地语、阿拉伯语、法语、葡萄牙语、日语、韩语作为候选语言提示，但必须由用户确认或选择。

多语言 README 可用两种组织方式：

| 方式 | 适用场景 | 写法 |
|------|----------|------|
| 多文件链接 | 默认选择；公开 README、内容超过一个屏幕、包含 Quick Start/表格/图表 | `README.md` + `README.zh-CN.md` / `README.es.md` 等，顶部互链 |
| 单文件锚点 | 很短的说明页或临时草稿 | 顶部语言锚点，所有语言段落结构一致 |

规则：

- 语言切换放在标题区附近。
- 各语言的 Quick Start、链接和文件定位必须同步。
- 不要一边有完整文档，另一边只有摘要。
- 不要用机器翻译腔补长篇解释；宁可简短但准确。
- 阿拉伯语等 RTL 语言可用独立文件，必要时在顶层容器或说明中标注方向；不要把 RTL 与 LTR 长文混在一个单文件 README 里。

## 多语言文件策略

公开 README 默认不要写成多个完整语言版本堆叠的长单文件。这样会让首屏变重、读者滚动成本变高，也容易在后续维护时出现语言 drift。

推荐做法：

- `README.md` 放主语言。国际项目通常用英文；中文项目可用中文，并提供 `README.en.md`。
- 其他语言使用清晰后缀，例如 `README.zh-CN.md`、`README.es.md`、`README.hi.md`、`README.ar.md`、`README.fr.md`、`README.pt.md`、`README.ja.md`、`README.ko.md`。
- 所有语言文件顶部互链，例如 `English · 简体中文 · Español · हिन्दी · العربية · Français · Português · 日本語 · 한국어`。
- 各文件保持相同章节顺序、安装命令、场景表和许可证链接。
- 只有极短 README 或一次性说明页，才允许用单文件锚点多语言。
